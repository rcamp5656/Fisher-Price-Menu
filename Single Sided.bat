echo off
cls
echo Batch File Preperation file for Fisher Price Records written By Russ Campbell Batch file Update on September 28 2024 at 12 Noon
...
echo ;
echo This batch file will ask you for 9 different MIDI files to import Music Box Mania. Without using the website Music Box Mania and setting the 
...
set /p UserInputFile9=Please enter the nineth  filename [Without Extender] 
:nineth
set /p UserTempo9=Enter The Tempo for the 9th  MIDI File   : 
%path%\src\music_box_tracker.py --maniacs %path%\maniacs\%UserInputFile9%.mid --audio midi --bpm %UserTempo9% --low --title %UserInputFile9% --port 1
set /p YesNo=Refine tempo of %UserInputFile9% [Y/N] : 
if %YesNo%==Y goto ninth
if %YesNo%==y goto ninth
echo ;
cls
echo ;
...
copy %path%\maniacs\%UserInputFile9%.fpr %path%\fpr\%UserInputFile9%.fpr
echo ;
...
%path%\src\fpr_to_scad.py --fpr %path%\fpr\%UserInputFile9%.fpr --scad %path%\scad\%UserInputFile9%.scad --thickness 3
echo ;
